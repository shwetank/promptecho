...
Practical recipes for using tapedeck in real codebases — from a one-engineer
...
Install:
```bash
pip install -e path/to/tapedeck   # not yet on PyPI
```
...
def test_extracts_invoice_total(tapedeck_cassette):
...
```bash
TAPEDECK_MODE=all pytest tests/test_extract.py   # re-records everything
```
...
def test_summarize(llm, tapedeck_cassette):
...
```python
@tapedeck.use_cassette("cassettes/demo_walkthrough.yaml", mode="none")
def demo_script():
...
import os, pytest, tapedeck

@pytest.fixture(autouse=True)
def _ci_mode_none(tapedeck_cassette):
    """In CI, every test that uses the tapedeck fixture must replay, never
    record. A cassette miss fails the build instead of making a live call."""
    yield  # fixture already enforces this when CI=true
```

The shipped `tapedeck_cassette` fixture already defaults to `mode="none"` when
...
This isn't tapedeck-specific but pairs with it well: any test that doesn't have
...
Same auth, same model, same backend — but now httpx-based and tapedeck sees it.
...
```python
# run.py
@tapedeck.use_cassette(f"cassettes/{CONDITION}.yaml")
...
def record_baselines():
    for provider in ["openai", "anthropic", "openrouter_llama"]:
        with tapedeck.use_cassette(f"cassettes/{provider}.yaml", mode="once"):
...
def analyze():
    ...
    for provider in ["openai", "anthropic", "openrouter_llama"]:
        with tapedeck.use_cassette(f"cassettes/{provider}.yaml", mode="none"):
            for prompt in BENCHMARK_PROMPTS:
...
# top of the notebook
import tapedeck
ctx = tapedeck.use_cassette("cassettes/notebook_session.yaml", mode="once")
...
@pytest.fixture
def tapedeck_cassette(request, tmp_path_factory):
    ...
    mode = os.environ.get("TAPEDECK_MODE", "none" if os.environ.get("CI") else "once")
    with tapedeck.use_cassette(path, mode=mode) as c:
        yield c
```

Then: `TAPEDECK_MODE=all pytest tests/test_thing.py` re-records that file.
...
```python
@tapedeck.use_cassette("cassettes/o3_high.yaml")
def test_o3_high():
...
@tapedeck.use_cassette("cassettes/o3_low.yaml")
def test_o3_low():
    r = OpenAI().chat.completions.create(
...
a content block). It's just JSON — tapedeck records the request normally and
the prompt-with-image becomes part of the fingerprint.

**Image output / Claude image / GPT-4o image.** The image is base64 inside a
JSON content block in the response. tapedeck records the JSON; the base64
...
**Don't put secrets in the prompt body and rely on tapedeck to redact them.**
...
reintroduces the exact non-determinism you adopted tapedeck to remove.

**Don't share one cassette across many unrelated tests.** Use the
`tapedeck_cassette` fixture or one file per test. Sharing a cassette across
tests works mechanically (the fingerprints find the right interaction) but
makes the cassette diff in PR review unreadable.

**Don't intercept and mock LLM responses *in addition to* tapedeck.** You'll
either double-mock (tapedeck records the mock, not the real response) or
fight over the httpx transport. Pick one. tapedeck is for the cases where you
...
- [DESIGN.md](DESIGN.md) — *why* tapedeck makes the choices it does
