...
The mechanism: [`patch.py`](src/tapedeck/patch.py) monkeypatches
...
The decision logic itself lives in [`transport.py`](src/tapedeck/transport.py) and
...
tapedeck weren't installed. That's a deliberate v1 scope. The roadmap item
...
non-determinism into the exact thing you adopted tapedeck to make deterministic,
...
— see the README's "What tapedeck is not." A `fuzzy=True` dev-loop convenience
...
[`normalizers.py`](src/tapedeck/normalizers.py) maps each raw provider body into
...
tapedeck captures the **ordered list of SSE events** as they arrive, stores them
...
(`REDACT_HEADERS` in [`cassette.py`](src/tapedeck/cassette.py)) — extend it for
